...
save_file = "PopulationNoiseResults_equalPasses.json"
const = False
data = "audio"
...
for p in pop_size[::1]: #[::int((this_comp - 0.5) * 2)]
    ...
    for j in range(5):
        ...
        for i in range(passes_max // passes_min):
            ...
            ## Train and Predict ##################################################################################
            print("Training...")
            lr = 0.002 #max(0.002, 0.02 * math.sqrt(p) / (i + 1))
            model.population_fit(efg.batch_audio_generator,
                                 epochs = epochs,
                                 population = p,
                                 learning_rate = lr * math.sqrt(p), #max(0.001, 0.002 * 10 / (i + 1)),
