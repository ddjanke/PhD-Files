...
wd_values = np.concatenate([np.array([0]),np.logspace(-5, 0, 21)])
zero_fill = len(str(len(wd_values) - 1))

try:
    raise TypeError()
    with open("L2Norm_w_Noise.json", 'r') as jf:
        trials = json.load(jf)
...
for nn_shape in [(bands * len(features), 15, 9, 3, 1),
    ...
    for i,wd in enumerate(wd_values):
        for j in range(5):
            ...
            trials[trial_name] = {"weight_decay" : wd,
                                  "nodes" : nn_shape,
            ...
            print()
            del model
            with open("L2Norm_w_Noise.json", 'w') as jf:
                    json.dump(trials, jf)
