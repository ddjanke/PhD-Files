...
if __name__ == "__main__":
    ...
    for strategy in ["all"]: #, "allsoft"]:
        ...
        for nn_shape in shapes:
            ...
            for sparsity in [5]: #("all",): #, "allsoft")[::1]:
                s = strategy if strategy == "allsoft" else "all"
                filename = save_file.format(nn_shape[1], len(nn_shape) - 2, s)
                first_check = True
                ...
                for mask_rate in [10,20]: #, 50, 100]:
                    if strategy == "once":
                        ...
                        mask_start = 0
                        if strategy == "allsoft" and mask_rate > 20: continue
                    ...
                    for p in [1,2,4,8]:
                        ...
                        for n in range(50):
                            ...
                            if strategy == "all":
                                trial_name = "{}_{}_{}_{}".format(str(nn_shape), strategy + str(sparsity), mask_rate, str(n))
                            elif strategy == "once":
                                trial_name = "{}_{}_{}_{}".format(str(nn_shape), strategy, mask_start, str(n))
                            elif strategy == "allsoft":
                                trial_name = "{}_{}_{}_{}_{}".format(str(nn_shape), strategy + str(sparsity), str(p), mask_rate, str(n))
                            ...
                            model = ann.ANNSparse(list(nn_shape), device="cuda", activation="CompTanh", sparsity=sparsity,
                                          strategy = strategy, mask_start = mask_start, Nmasks = 20,
                            ...
                            if strategy == "allsoft":
                                ...
                                while not model.es.stop:
                                    loss = model.fit(datagen, epochs=epochs_each, learning_rate = 0.001 ** (goal_loss/loss),
                                              early_stop_train = True, generator_kwargs=train_kwargs, verbose=10)
                                model.es = ann.myEarlyStop(patience = 100, delta = 0.000001, mode = "loss")
